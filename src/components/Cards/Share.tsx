...
const ShareCard = () => {
        ...
        if (shareWithEmail && !shareWithEveryone) {
          ...
          axios
            ...
            .then((response) => {
              ...
              setMessage({open: true, text: response.data.message, type: response.data.type});
              setTimeout(() => {
  ...
  return (
    <div className="absolute inset-0 z-50 mx-auto md:w-1/4 w-3/5 h-72 mt-24 rounded-xl flex flex-col items-center" style={{ backgroundColor: "#2D4A53" }}>
        <div className="flex flex-row justify-center w-full">
        <label className="mt-8 mb-4 mx-4 text-xl font-medium text-gray-900 dark:text-white">Share</label>
        {!optionSelect && <label className="inline-flex hover:text-gray-300 w-28  h-9 py-2 px-4 mt-8 text-sm font-medium text-gray-900 bg-white border border-gray-200 rounded-lg dark:bg-gray-700 dark:border-gray-600 dark:text-white" onClick={()=>setOptionSelect(true)}>
        ...
        {optionSelect && !shareWithEveryone && <ul className="mt-8 w-28 text-sm font-medium text-gray-900 bg-white border border-gray-200 rounded-lg dark:bg-gray-700 dark:border-gray-600 dark:text-white">
            <li style={{ cursor: "pointer" }} key="friend" className="px-4 py-2 hover:text-gray-300 border-b border-gray-200 dark:border-gray-600" onClick={()=>{setShareWithEveryone(false); setOptionSelect(false);}}>friend</li>
            <li style={{ cursor: "pointer" }} key="everyone" className="px-4 py-2 hover:text-gray-300 border-b border-gray-200 dark:border-gray-600" onClick={()=>{setShareWithEveryone(true); setOptionSelect(false);}}>everyone</li>
        </ul>}
        {optionSelect && shareWithEveryone && <ul className="mt-8 w-28 text-sm font-medium text-gray-900 bg-white border border-gray-200 rounded-lg dark:bg-gray-700 dark:border-gray-600 dark:text-white">
            <li style={{ cursor: "pointer" }} key="everyone" className="px-4 py-2 hover:text-gray-300 border-b border-gray-200 dark:border-gray-600" onClick={()=>{setShareWithEveryone(true); setOptionSelect(false);}}>everyone</li>
            <li style={{ cursor: "pointer" }} key="friend" className="px-4 py-2 hover:text-gray-300 border-b border-gray-200 dark:border-gray-600" onClick={()=>{setShareWithEveryone(false); setOptionSelect(false);}}>friend</li>
      ...
      {shareWithEveryone === false && (
        <div className={`${optionSelect?'w-full px-4':'w-full mt-8 px-4'}`}>
          <label className="text-left text-lg font-medium text-gray-900 dark:text-white">Email</label>
          <div className="mt-2">
            <input
      ...
      {shareWithEveryone ? (
        <div className={`${optionSelect?'w-full px-4':'w-full mt-8 px-4'}`}>
          ...
          <input id="text-box" className="w-full bg-gray-50 border border-white text text-gray-900 text rounded-lg w-60 p-2.5 dark:bg-gray-700 dark:text-white " value={shareLink} required></input>
